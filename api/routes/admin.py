...
_OCR_LOW_THRESHOLD = 0.6
_OCR_HIGH_THRESHOLD = 0.9


def _compute_material_ocr_stat(material: EvidenceMaterial) -> OcrMaterialStat:
    """从 evidence_materials.ocr_result JSONB 提取置信度统计"""
    ocr_result = material.ocr_result or {}
    blocks = ocr_result.get("blocks") or []
    source_type = ocr_result.get("source_type")

    if not blocks:
        return OcrMaterialStat(
            material_id=str(material.id),
            filename=material.original_filename,
            file_type=material.file_type,
            effective_category=material.effective_category,
            ...
            min_confidence=None,
            low_conf_count=0,
            char_count=len((material.ocr_text or "") or ocr_result.get("full_text", "")),
            has_blocks=False,
        )

    confidences = [b.get("confidence", 0.0) for b in blocks if isinstance(b, dict)]
    texts = [b.get("text", "") for b in blocks if isinstance(b, dict)]
    char_count = sum(len(t) for t in texts)

    avg_conf = sum(confidences) / len(confidences) if confidences else None
    min_conf = min(confidences) if confidences else None
    low_count = sum(1 for c in confidences if c < _OCR_LOW_THRESHOLD)
    ...
    return OcrMaterialStat(
        ...
        min_confidence=round(min_conf, 4) if min_conf is not None else None,
...
def _check_field_hits(full_text: str) -> set[str]:
    """检查 full_text 中命中了哪些关键字段"""
    if not full_text:
        return set()
    return {kw for kw in _OCR_FIELD_KEYWORDS if kw in full_text}


@router.get("/ocr-monitor", response_model=OcrMonitorResponse)
@limiter.limit("5/minute")
async def ocr_monitor(
    request: Request,
    current_user: User = Depends(get_current_user),
    db: AsyncSession = Depends(get_db),
):
    """OCR 质量监控总览（仅 super_admin）

    扫描全部案件的 evidence_materials.ocr_result JSONB，
    返回每案件/每材料的 OCR 置信度统计、质量分布、关键字段命中率。
    """
    if not _require_super_admin(current_user):
        raise HTTPException(
            status_code=status.HTTP_403_FORBIDDEN,
    ...
    # 拉取所有案件 + 材料（super_admin 看全局）
    cases = (
        await db.execute(
            select(EvidenceCase).order_by(EvidenceCase.created_at.desc())
        )
    ).scalars().all()
    case_ids = [c.id for c in cases]

    # 一次性拉所有材料，按 case_id 分组（避免 N+1）
    materials_by_case: dict[uuid.UUID, list[EvidenceMaterial]] = {}
    if case_ids:
        all_materials = (
            await db.execute(
                select(EvidenceMaterial)
                .where(EvidenceMaterial.evidence_case_id.in_(case_ids))
                .order_by(EvidenceMaterial.created_at.asc())
            )
        ).scalars().all()
        for m in all_materials:
            materials_by_case.setdefault(m.evidence_case_id, []).append(m)

    # 租户名映射（用于显示）
    tenant_ids = {c.tenant_id for c in cases if c.tenant_id is not None}
    tenant_names: dict = {}
    if tenant_ids:
        tenant_rows = (
            await db.execute(
        ...
        tenant_names = {row[0]: row[1] for row in tenant_rows}

    # 字段命中率统计
    field_hit_counts = {kw: 0 for kw in _OCR_FIELD_KEYWORDS}
    field_total_with_text = 0

    # 全局聚合
    total_materials = 0
    materials_with_ocr = 0
    ocr_completed = 0
    ocr_failed = 0
    ocr_pending = 0
    all_confidences: list[float] = []
    quality_dist = {"high": 0, "medium": 0, "low": 0, "no_data": 0}
    source_type_stats: dict[str, int] = {}

    case_stats: list[OcrCaseStat] = []

    for case in cases:
        mats = materials_by_case.get(case.id, [])
        mat_stats: list[OcrMaterialStat] = []
        case_confidences: list[float] = []
        case_low_quality = 0

        for m in mats:
            stat = _compute_material_ocr_stat(m)
            mat_stats.append(stat)
            total_materials += 1
            ...
            if stat.has_blocks:
                materials_with_ocr += 1
                if stat.avg_confidence is not None:
                    case_confidences.append(stat.avg_confidence)
                    all_confidences.append(stat.avg_confidence)
                    if stat.avg_confidence < _OCR_LOW_THRESHOLD:
                        quality_dist["low"] += 1
                        case_low_quality += 1
                    elif stat.avg_confidence >= _OCR_HIGH_THRESHOLD:
                        quality_dist["high"] += 1
                    else:
                        quality_dist["medium"] += 1
                if stat.source_type:
                    source_type_stats[stat.source_type] = source_type_stats.get(stat.source_type, 0) + 1
            else:
                quality_dist["no_data"] += 1

            full_text = (m.ocr_text or "") or ((m.ocr_result or {}).get("full_text", ""))
            if full_text:
                field_total_with_text += 1
                hits = _check_field_hits(full_text)
                for kw in hits:
                    field_hit_counts[kw] += 1

        case_avg = sum(case_confidences) / len(case_confidences) if case_confidences else None
        case_stats.append(OcrCaseStat(
            case_id=str(case.id),
            case_name=case.case_name,
            case_type=case.case_type,
            is_minor=bool(case.is_minor),
            tenant_id=str(case.tenant_id) if case.tenant_id else None,
            tenant_name=tenant_names.get(case.tenant_id) if case.tenant_id else None,
            material_count=len(mats),
            ocr_completed=sum(1 for m in mats if m.ocr_status == "completed"),
            ocr_failed=sum(1 for m in mats if m.ocr_status == "failed"),
            avg_confidence=round(case_avg, 4) if case_avg is not None else None,
            low_quality_count=case_low_quality,
            materials=mat_stats,
        ))

    global_avg = sum(all_confidences) / len(all_confidences) if all_confidences else None
    global_low = quality_dist["low"]
    global_high = quality_dist["high"]

    field_hit_rates = {
        kw: round(field_hit_counts[kw] / field_total_with_text, 4)
        for kw in _OCR_FIELD_KEYWORDS
        if field_total_with_text > 0
    }

    return OcrMonitorResponse(
        scope="global",
        total_cases=len(cases),
        total_materials=total_materials,
        materials_with_ocr=materials_with_ocr,
        ocr_completed=ocr_completed,
        ocr_failed=ocr_failed,
        ocr_pending=ocr_pending,
        avg_confidence=round(global_avg, 4) if global_avg is not None else None,
        low_quality_count=global_low,
        high_quality_count=global_high,
